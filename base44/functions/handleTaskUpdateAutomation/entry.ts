...
Deno.serve(async (req) => {
  try {
    ...
    // Only notify for meaningful field changes
    const relevantFields = ['title', 'description', 'assigned_to', 'due_date', 'status', 'priority', 'column_id', 'tags'];
    const hasRelevantChange = (changed_fields || []).some(f => relevantFields.includes(f));

    if (!hasRelevantChange) {
      return Response.json({ message: 'No relevant changes, skipping notification' });
    }
    ...
    // We don't know who made the update in an automation, so notify all watchers + assignee
    const recipientEmails = new Set();
    if (task.assigned_to) recipientEmails.add(task.assigned_to);
    if (task.watchers && Array.isArray(task.watchers)) {
      task.watchers.forEach(email => recipientEmails.add(email));
    }

    if (recipientEmails.size === 0) {
      return Response.json({ message: 'No recipients' });
    }
    ...
    const fieldLabels = {
      title: 'Title', description: 'Description', assigned_to: 'Assignee',
      due_date: 'Due Date', status: 'Status', priority: 'Priority',
      column_id: 'Column / Stage', tags: 'Tags',
    };

    const changedLabels = (changed_fields || [])
      .filter(f => relevantFields.includes(f))
    ...
    const sendPromises = Array.from(recipientEmails).map(async (email) => {
      ...
      const html = buildEmailHtml({
        ...
        introHtml: `The task <strong>${task.title}</strong> has been updated.<br/>${changeDescription}`,
        conversationItems,
        ctaUrl: taskUrl,
      });
