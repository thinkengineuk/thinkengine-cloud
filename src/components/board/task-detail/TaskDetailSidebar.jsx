          ...
          <div className="flex items-center justify-between">
           ...
           <Dialog open={showWatchersDialog} onOpenChange={setShowWatchersDialog}>
             <DialogTrigger asChild>
               <Button variant="ghost" size="sm" className="h-8 text-xs px-2">Add</Button>
             </DialogTrigger>
             <DialogContent className="max-w-sm">
               <DialogHeader><DialogTitle>Add Watchers</DialogTitle></DialogHeader>
               <ScrollArea className="h-64">
                 <div className="space-y-2">
                   {(allUsers || []).filter(u => !(task.watchers || []).includes(u.email)).map((user) => (
